...
def fetch_unique_addresses(
    ...
    if df.empty:
        output = pd.DataFrame()
        maybe_save_csv(output, config.output_dir, "eth_unique_addresses.csv", enabled=save)
        return output

    if "UnixTimeStamp" not in df.columns or "Value" not in df.columns:
        raise RuntimeError("Expected columns not found in unique address chart.")

    df["timestamp"] = pd.to_numeric(df["UnixTimeStamp"], errors="coerce")
    df["total_unique_addresses"] = pd.to_numeric(df["Value"], errors="coerce")

    df = _filter_range(df, start_ts, end_ts)
    df = df.sort_values("timestamp")
    df["daily_increase"] = df["total_unique_addresses"].diff().fillna(0)

    output = df[["timestamp", "total_unique_addresses", "daily_increase"]].copy()
    if as_unix:
