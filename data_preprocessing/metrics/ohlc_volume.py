...
def fetch_eth_ohlc_volume(
    ...
    raw = fetch_coinbase_candles(
        product_id=config.eth_product_id,
        start=start_ts,
        end=end_ts,
    ...
    if raw.empty:
        ...
        df = format_ohlc_frame(raw, config.eth_product_id, caller)

    output = df.copy()
    if as_unix:
        output = to_unix_timestamp(output, "timestamp")

    maybe_save_csv(output, config.output_dir, "eth_ohlc_volume.csv", enabled=save)
    return output
